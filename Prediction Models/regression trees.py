"""This code performs decision tree regression on the dataframes imported from the data_prep.py file. A function is made for the regression tree model and
...
is performed inside the function to try to reduce overfitting by finding optimum parameters for the model. Another layer of cross-validation is performed to find the mean
of the model metrics over the 10 folds. 
...
"""function to perform Decision Tree Regression on the day-wise dataframes and plot the results"""
df_regressiontree_results = pd.DataFrame() # Create a new dataframe to store the results of the regression tree
def ols_linear_regression_days (dataframe, audience_name):
    global df_regressiontree_results

    max_days = dataframe["Days to Event"].max()
    dataframe["Days to Event"] = max_days - dataframe["Days to Event"] # Reverse the order of the days to event to start from 0
    ...
    tree = DecisionTreeRegressor() # Create a regression tree model

    grid_search = GridSearchCV(estimator = tree, param_grid = parameter_grid, cv = 10, scoring = "neg_mean_squared_error") # Create a grid search object
    grid_search.fit(X, y) # Fit the grid search object to the data
    best_params = grid_search.best_params_ # Get the best parameters from the grid search

    regr = DecisionTreeRegressor(max_depth = best_params["max_depth"], min_samples_split = best_params["min_samples_split"], min_samples_leaf = best_params["min_samples_leaf"]) # Create a new regression tree model with the best parameters
    ...
    X_testplot_df = pd.DataFrame(X_testplot, columns = ["Days to Event", "Advertisement Status"]) # Create a new dataframe for the test set
    y_testplot_series = pd.Series(y_testplot, name = "Booking Count") # Create a new series for the test set
    ...
    plt.title("Decision Tree Regression for " + audience_name) # Set the title of the plot
    ...
    df_regressiontree_results = pd.concat([df_regressiontree_results, new_row], ignore_index = True) # Add the results to the dataframe
...
print(df_regressiontree_results)
...
try:
    df_regressiontree_results.to_csv(os.path.join(full_path, "regression_tree.csv")) # Save the results to a csv file in the subfolder
